...
def obter_dashboard():
    ...
    domingo_id, data_domingo, sem_aula = domingo

    # Quantidade de domingos cadastrados
    cursor.execute("""
        SELECT COUNT(*)
        FROM domingos
    """)

    total_domingos = cursor.fetchone()[0] or 0

    # Resumo geral do último domingo
    cursor.execute("""
        SELECT
            COALESCE(SUM(matriculados), 0),
            COALESCE(SUM(presentes), 0),
            COALESCE(SUM(ausentes), 0),
            COALESCE(SUM(visitantes), 0),
            COALESCE(SUM(biblias), 0),
            COALESCE(SUM(revistas), 0),
            COALESCE(SUM(oferta), 0)
        FROM registros_classe
        WHERE domingo_id = ?
    """, (domingo_id,))
    ...
    cursor.execute("""
        SELECT
            classes.id,
            classes.nome,
            COALESCE(registros_classe.matriculados, 0),
            COALESCE(registros_classe.presentes, 0),
            COALESCE(registros_classe.ausentes, 0),
            COALESCE(registros_classe.visitantes, 0)
        FROM classes
        LEFT JOIN registros_classe
            ON registros_classe.classe_id = classes.id
            AND registros_classe.domingo_id = ?
        ORDER BY classes.id
    """, (domingo_id,))
    ...
    for registro in cursor.fetchall():
        classes.append({
            "id": registro[0],
            "nome": registro[1],
            "matriculados": registro[2],
            "presentes": registro[3],
            "ausentes": registro[4],
            "visitantes": registro[5]
        })
